...
Auf ILIAS findet ihr im Ordner *Individuelle Daten Abschlussarbeit* einen Ordner mit eurem Namen. Dieser ist nach dem PsychDS-Systemstrukturiert und enthält im Unterordner *raw* eure persönlichen, simulierten Datensätze für die Abschlussarbeit.
...
-   Unterscheiden sich zwei unabhängige Gruppen voneinander?

-   Verändert sich eine Variable innerhalb derselben Personen über zwei Messzeitpunkte?
...
![](../../images/EH_11/T-Test.png){fig-align="center" width="400"}
...
**Fragestellung:** Unterscheiden sich Personen in der `above`- und `below`-Gruppe darin, wie sie ihre kommende Leistung im Pattern Copy Task im Vergleich zu anderen Studierenden einschätzen?
...
| t-Test für unabhängige Stichproben | Unterscheiden sich zwei unabhängige Gruppen? | `t.test(av ~ uv, data = dat)` |
| Gepaarter t-Test | Unterscheiden sich zwei Messzeitpunkte derselben Personen? | `t.test(av1, av2, paired = TRUE)` |

<br>

**Für unser Beispiel:**

Wir verwenden einen t-Test für unabhängige Stichproben.

```{r, eval=FALSE, echo=TRUE}
t.test(pre4 ~ group_all, data = dat_full)
```
...
-   p \< .05: Hinweis auf unterschiedliche Varianzen (Welch-t-Test verwenden)
...
Wenn ihr unsicher seid, verwendet den Welch-t-Test. Er ist bei ungleichen Varianzen robuster.
...
-   Links steht die abhängige Variable: `pre4`

-   Rechts steht die Gruppierungsvariable: `group_all`

-   R vergleicht die Mittelwerte von `pre4` zwischen den Gruppen `above` und `below`
...
**Aber:**\
Der p-Wert sagt noch nicht, wie gross der Unterschied ist.

Dafür brauchen wir zusätzlich eine Effektgrösse.
...
Er sagt aber nicht direkt, wie gross dieser Unterscheid ist.
