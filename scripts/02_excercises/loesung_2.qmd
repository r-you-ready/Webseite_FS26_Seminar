...
# Lösung - Coding Basics (Einheiten 3 und 4)
...
Üblicherweise eignen sich relative Pfade besser als absolute Pfade, da diese unabhängig vom Computer funktionieren, auf dem das Skript ausgeführt wird. Dabei muss allerdings darauf geachtet werden die Projektstruktur gleich zu halten - deshalb empfiehlt es sich allgemeinen Standards wie dem PsychDS zu folgen.
:::
...
::: {.callout-note collapse="true" title="Lösung"}
```{r}
# install.packages("styler") # den Code führst du einmalig in deiner Console aus
library(styler)
```
:::
