...
# Hands On - Coding Basics (Einheiten 3 und 4)
...
Wir verwenden Funktionen aus den Paketen:

    - **readr** (für .csv)
    - **readxl** (für .xlsx)
    
Beide sind Teil des Tidyverse und zentrale Werkzeuge für sauberen und reproduzierbaren Datenimport.

### Einlesen via Oberfläche (Point & Click)

Gerade zu Beginn kann die Import-Oberfläche von RStudio hilfreich sein.
...
1. Importiere die Datei `"data_mmq.csv"` über die Point & Klick-Oberfläche von RStudio. Verwende dafür "From Text (readr)"
...
**Ganz wichtig: Der erzeugte Code**

Die Oberfläche ist nur ein Hilfsmittel.
Entscheidend ist der **Code**, der dabei generiert wird.
...
Langfristig solltest du Daten **direkt per Code** einlesen - ohne Klick-Oberfläche.
...
-   Tipp: Installiere und lade das Paket zunächst.
...
-   Nutze bei Unsicherheiten `?read_excel` oder das Cheatsheet.
...
💡 Reflektiere:
Macht das Sinn? Welche Gefahr entsteht, wenn Daten nur positionsbasiert zusammengefügt werden?
...
-   Nenne diesen `dat_full`
    ...
    👉  Hinweis: Im Datensatz `data_pct` heisst die Code-Variable leicht anders. Verwende deshalb `by = c("code_all" = "code")`.
...
👉 Für diesen zusammengeführten Datensatz werdet ihr anschliessend das **Codebook** erstellen für die Abschlussarbeit.
...
Die Defaults findest du in der Hilfe:
-    im Tab *Help*
-    oder in der Konsole mit `?Funktionsname` (z.B. `?sort`)
...
Um sicher die richtige Version zu verwenden:
