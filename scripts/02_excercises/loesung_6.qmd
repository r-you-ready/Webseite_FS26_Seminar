...
✅ einfache Visualisierungen mit `ggplot2` systematisch aufbauen
...
# mmq_mean auf Personenebene
# in eurem Datensatz nun (wahrscheinlich) schon enthalten
dat_full <- dat_full |> 
  mutate(mmq_mean = rowMeans(select(dat_full, starts_with("question")), na.rm = TRUE))
...
Füge mit `labs()` einen Titel sowie Achsenbeschriftungen hinzu. Verändere ausserdem `theme_classic().`
...
::: {.callout-note collapse="true"}
Im Datensatz liegen die Ratings als Werte zwischen 0 und 10 vor. Für die Darstellung als Percentile Rank werden Mittelwert, Standardabweichung und Standardfehler mit 10 multipliziert.
:::
...
descriptives <- dat_long |> 
  ...
  summarize(
    N = sum(is.na(rating)),
    mean_d = mean(XXXX, na.rm = TRUE) * 10,
    sd_d   = sd(XXXX, na.rm = TRUE) * 10,
    se_d   = sd_d / sqrt(N),
    .groups = "drop"
)
...
descriptives <- dat_long |> 
  ...
  summarize(
    N = sum(ïs.na(rating)),
    mean_d = mean(rating, na.rm = TRUE) * 10,
    sd_d   = sd(rating, na.rm = TRUE) * 10,
    se_d   = sd_d / sqrt(N),
    .groups = "drop"
)
...
Berechne nun die Korrelation mit `cor.test()`.

```{r, echo = TRUE, eval = FALSE}
cor.test(dat_full$XXXX, dat_full$XXXX)
```
...
```{r, echo = TRUE, eval = FALSE}
cor.test(dat_full$mean_d1_all, dat_full$mean_d_all, alternative = "XXXX")
cor.test(dat_full$mean_d1_all, dat_full$mean_d_all, method = "XXXX")
```
...
Das Modell erklärt etwa 64% der Varianz in mean_d1_all (R² = 0.64), was auf einen starken Zusammenhang hinweist.

👉 Insgesamt zeigt sich also: Personen mit längerer Gesamtbearbeitungszeit haben tendenziell auch eine längere Zeit der ersten Fensteröffnung.
```
:::
...
### 6.2 Interpretiere das multiple Modell
...
5.  Wie unterscheidet sich `R-squared` im Vergleich zu `model_1`?

6.  Hier müssen wir auf Multikollinearität testen – mit `vif(model)` aus dem „car“ Paket (die Werte sollten größer als 0.10 sein, sodass keine Multikollinearität vorliegt). Berechne auch die Konfidenzintervalle der Beta-Koeffizienten mit `confint(model)`.
...
### 6.3 Prüfe Multikollinearität mit `vif()`
...
Ein 95%-Konfidenzintervall beschreibt einen Bereich plausibler Werte für den wahren Regressionskoeffizienten. Wenn das 95%-KI die 0 nicht enthält, passt das zu einem zweiseitig signifiknten Test auf dem 5%.Niveau.
...
Wir vergleichen `pre4` zwischen den Gruppen `above` und `below`.

-   Starte mit einem `leveneTest()` aus dem `„car“` Paket um die Varianzhomogenität zu prüfen. Tipp: Filtere die Daten, sodass nur die beiden Gruppen `„above“` und `„below“` beinhaltet sind (d.h. ohne die Gruppe `„control“`).
